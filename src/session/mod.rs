...
impl Session {
    ...
    pub fn new_with_options(
        ...
        let client = fetch::build_client_h1_fallback(
            Some(user_agent.unwrap_or(DEFAULT_USER_AGENT)),
            jar.clone(),
            tls_config,
        )
        ...
        let mut fetch_result = match action {
            ...
            InterceptAction::Continue(overrides) => {
                ...
                if extra_headers.is_empty() {
                    fetch::fetch_url(&self.client, actual_url, 30000)
                        .await
                        .map_err(|e| {
                            self.status = SessionStatus::Idle;
                            e.to_string()
                    ...
                    fetch::fetch_url_with_headers(&self.client, actual_url, 30000, &extra_headers)
                        .await
                        .map_err(|e| {
                            self.status = SessionStatus::Idle;
                            e.to_string()
    ...
    pub async fn create_session_with_options(
        ...
        if self.limits.total_memory_budget > 0 {
            ...
            if total >= self.limits.total_memory_budget {
                return Err(format!(
                    "Total memory budget ({} bytes) exceeded",
                    self.limits.total_memory_budget
                ));
            }
        }
        ...
        sessions.insert(
            id.clone(),
            Session::new_with_options(id.clone(), user_agent, auth_profiles)?,
        );
...
async fn fetch_single(
    ...
    auth_profiles: &[String],
    pipeline_config: &PipelineConfig,
) -> Result<ParallelFetchResult, String> {
    let start = Instant::now();
    let jar = Arc::new(Jar::default());
    for domain in auth_profiles {
        let _ = crate::auth::store::load_into_jar(domain, &jar);
    }
    let client = fetch::build_client_h1_fallback(None, jar, crate::network::tls::global())
        .map_err(|e| e.to_string())?;
    let fetch_result = fetch::fetch_url(&client, url, 30000)
        .await
        .map_err(|e| e.to_string())?;
    let fetch_ms = fetch_result.load_ms;
    let html_bytes = fetch_result.html_bytes;
    ...
    Ok(ParallelFetchResult {
        ...
        fetch_ms,
        pipeline_ms: (page_result.timing.total_us / 1000) as u64,
...
mod tests {
    ...
    async fn test_max_sessions() {
        let m = SessionManager::new(ResourceLimits {
            max_sessions: 2,
            ..Default::default()
        });
