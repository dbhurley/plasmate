            ...
            Ok(tokio_tungstenite::tungstenite::Message::Text(text)) => {
                ...
                if req.method == "Target.attachToTarget" {
                    ...
                    let found = if default_target.target_id == attach_target_id {
                        Some((
                            default_target.session_id.clone(),
                            default_target.target_id.clone(),
                            default_target
                                .current_url
                                .as_deref()
                        ...
                        extra_targets
                            .values()
                            .find(|t| t.target_id == attach_target_id)
                            .map(|t| {
                                (
                                    t.session_id.clone(),
                                    t.target_id.clone(),
                    ...
                    if let Some((sid, tid, url)) = found {
                        let events = vec![super::types::CdpEvent::new(
